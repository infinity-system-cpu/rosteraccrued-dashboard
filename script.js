...
function inicializarApp() {
    ...
    const tieneSesion = checkExistingSession();
...
function logout() {
    ...
    // Limpiar timeouts
    logoutTimeouts.forEach(t => clearTimeout(t));
    logoutTimeouts = [];
    
    // Mostrar loading
    mostrarLoadingCierre();
    ...
    setTimeout(() => {
        try {
            ...
            currentTecnicoNomina = null;
            currentDesdeNomina = null;
            currentHastaNomina = null;
            folioEnEdicion = null;
            
            // 2. LIMPIAR LOCALSTORAGE
            localStorage.clear();
            
            // 3. OCULTAR APP CON TRANSICIÓN SUAVE
            const appElement = document.getElementById('app');
            if (appElement) {
            ...
            if (loginSection) {
                ...
                setTimeout(() => {
                    ...
                    const usernameInput = loginSection.querySelector('input[name="usuario"]');
                    if (usernameInput) {
            ...
            // 5. Resetear formularios
            const forms = document.querySelectorAll('form');
            forms.forEach(form => form.reset());
            
            // 6. Cerrar menú si está abierto
            const nav = document.getElementById('side-menu');
            if (nav && nav.classList.contains('active')) {
                nav.classList.remove('active');
            }
...
function limpiarEstilosInline() {
    console.log("Limpiando estilos inline...");
    
    // Limpiar estilos inline de todos los elementos principales
    ...
    elementosPrincipales.forEach(element => {
        if (element) {
            // Remover todos los estilos inline
            element.removeAttribute('style');
            
            // Remover clases específicas que podrían interferir
            element.classList.remove('force-visible', 'hidden-by-role', 'menu-open');
            
            // Restaurar clases base según el elemento
            if (element.id === 'app') {
                element.style.cssText = 'display: none;';
            }
            
            if (element.id === 'login-section') {
                element.style.cssText = 'display: flex; visibility: visible; opacity: 1;';
            }
        }
    });
    
    // También limpiar estilos del body
    document.body.removeAttribute('style');
    document.body.className = '';
    document.documentElement.removeAttribute('style');
}

function limpiarEstadoResidual() {
    console.log("=== LIMPIANDO ESTADO RESIDUAL ===");
    
    // 1. Asegurar que NO haya elementos de la app visibles
    const appElements = [
        document.getElementById('app'),
        document.getElementById('side-menu'),
        document.querySelector('.top-bar'),
    ...
    appElements.forEach(element => {
        if (element) {
            ...
            element.classList.remove('active', 'force-visible');
        }
    });
    
    // 2. Asegurar que el login sea completamente visible
    const loginSection = document.getElementById('login-section');
    if (loginSection) {
        loginSection.style.display = 'flex';
        loginSection.style.visibility = 'visible';
        ...
        if (loginForm) {
            ...
            // Enfocar campo de usuario
            const usernameInput = loginForm.querySelector('input[name="usuario"]');
            if (usernameInput) {
                setTimeout(() => {
                    usernameInput.focus();
                }, 100);
    ...
    // 3. Resetear scroll completamente
    document.body.style.overflow = 'auto';
    ...
    bodyClasses.remove('scroll-locked', 'menu-active', 'modal-open', 'loading-active', 'loading-complete', 'force-visible');
    
    // 4. Remover cualquier loading overlay residual
    ...
    // 6. Limpiar intervalos o timeouts pendientes
    // (Si tienes intervalos, deberías limpiarlos aquí)
    
    console.log("✅ Estado residual limpiado");
}

function mostrarLoadingCierre() {
    console.log("Mostrando loading de cierre...");
    
    // Primero limpiar cualquier loading existente
    const existingLoadings = document.querySelectorAll('.loading-overlay');
    existingLoadings.forEach(loading => {
        if (loading && loading.parentNode) {
            loading.parentNode.removeChild(loading);
        }
    });
    
    // Crear nuevo loading específico para cierre
    const loadingOverlay = document.createElement('div');
    loadingOverlay.id = 'loading-overlay-cierre';
    loadingOverlay.className = 'loading-overlay cierre-sesion';
    
    loadingOverlay.innerHTML = `
        <div class="telecom-spinner">
            <div class="spinner-outer"></div>
            <div class="spinner-middle"></div>
            <div class="spinner-inner"></div>
            <div class="spinner-center"></div>
            <div class="spinner-particles" id="spinner-particles-cierre"></div>
        </div>
        
        <div class="loading-text">
            <h3>Cerrando sesión</h3>
            <p>Limpiando datos de usuario...</p>
            <div class="dots">
                <div class="dot"></div>
                <div class="dot"></div>
                <div class="dot"></div>
        ...
        <div class="progress-container">
            <div class="progress-bar cierre-progress"></div>
        </div>
    `;
    
    document.body.appendChild(loadingOverlay);
    
    // Bloquear scroll durante el cierre
    document.body.style.overflow = 'hidden';
    ...
    setTimeout(() => {
        ...
        // Animar progreso
        const progressBar = loadingOverlay.querySelector('.cierre-progress');
        if (progressBar) {
        ...
        const textoMensaje = loadingOverlay.querySelector('.loading-text p');
        const mensajes = [
            "Cerrando sesión...",
        ...
        mensajes.forEach((mensaje, index) => {
            ...
            }, index * 400); // Cada 0.2 segundos
        });
        
    }, 50); //50
}

function ocultarLoadingCierre() {
    const loadingOverlay = document.getElementById('loading-overlay-cierre');
    if (loadingOverlay) {
    ...
    // También limpiar el loading principal si existe
    const mainLoading = document.getElementById('loading-overlay');
    if (mainLoading) {
        mainLoading.classList.remove('active');
        setTimeout(() => {
            if (mainLoading.parentNode) {
...
function updateMenuByRole() {
    ...
    console.log("🔧 Actualizando menú por rol...");
    console.log("Usuario actual:", currentUser.name, "Rol:", currentUser.role);
    ...
    const menuItems = {
        ...
        'base': ['gerente', 'administrativo']
    };

    console.log("Configuración de menú:", menuItems);

    const buttons = document.querySelectorAll('button[onclick]');
    console.log(`Encontrados ${buttons.length} botones con onclick`);
    ...
    buttons.forEach((btn, index) => {
        ...
        const match = onclickAttr ? onclickAttr.match(/showSection\(['"]([^'"]+)['"]\)/) : null;
        
        if (match) {
            const sectionId = match[1];
            console.log(`  ➡️ Sección ID extraída: "${sectionId}"`);
            
            const shouldShow = menuItems[sectionId] === true || 
                              (menuItems[sectionId] && menuItems[sectionId].includes(currentUser.role));
            
            console.log(`  ✅ Debe mostrarse: ${shouldShow}`);
            console.log(`  🎯 Rol permitido: ${menuItems[sectionId]}`);
            
            btn.style.display = shouldShow ? 'flex' : 'none';
            
            // También agregar/remover clase para mejor control
            if (shouldShow) {
            ...
            console.log("  ❌ No se pudo extraer sectionId");
        }
    });

    // También ocultar elementos li que contienen los botones
    const menuItemsList = document.querySelectorAll('.menu-item, .nav-item, li');
    menuItemsList.forEach(item => {
        const button = item.querySelector('button[onclick]');
        if (button) {
            item.style.display = button.style.display;
        }
    });

    populateUserDropdowns();
    console.log("✅ Menú actualizado por rol");
}
...
function mostrarLoading() {
    ...
    if (!loadingOverlay) {
        ...
        loadingOverlay.innerHTML = `
            <div class="telecom-spinner">
                ...
                <div class="spinner-inner"></div>
                <div class="spinner-center"></div>
                <div class="spinner-particles" id="spinner-particles"></div>
            </div>
            
            <div class="loading-text">
                <h3>Iniciando sesión</h3>
                <p>Espere un momento, cargando base de datos...</p>
                <div class="dots">
                    <div class="dot"></div>
                    <div class="dot"></div>
                    <div class="dot"></div>
        ...
        // Crear partículas alrededor del spinner
        crearParticulasSpinner();
    }
    ...
    // Crear temporizador de 4 segundos
    const tiempoTotal = 6000; // 4 segundos
    const mensajes = [
        "Conectando con el servidor...",
        "Verificando credenciales de acceso...",
        "Cargando base de datos de folios...",
    ...
    // Actualizar mensajes progresivamente
    const textoMensaje = loadingOverlay.querySelector('.loading-text p');
    const tiempoPorMensaje = tiempoTotal / mensajes.length;
    ...
    mensajes.forEach((mensaje, index) => {
        setTimeout(() => {
            if (textoMensaje) {
                ...
                textoMensaje.style.opacity = '0';
                setTimeout(() => {
                    textoMensaje.style.transition = 'opacity 0.3s ease';
                    textoMensaje.style.opacity = '1';
                }, 50);
            }
        }, index * tiempoPorMensaje);
    });
...
function crearParticulasSpinner() {
    const particlesContainer = document.getElementById('spinner-particles');
    if (!particlesContainer) return;
    
    particlesContainer.innerHTML = '';
    
    // Crear 8 partículas distribuidas
    for (let i = 0; i < 8; i++) {
        const particle = document.createElement('div');
        particle.className = 'spinner-particle';
...
function login(e) {
    ...
    if (user) {
        ...
        // 1. MOSTRAR LOADING
        mostrarLoadingInicio();
        
        // 2. Configurar usuario
        currentUser = user;
        localStorage.setItem('currentUser', JSON.stringify(user));
        
        // 3. OCULTAR LOGIN CON ANIMACIÓN
        const loginSection = document.getElementById('login-section');
        if (loginSection) {
            loginSection.style.opacity = '0';
        ...
        // 4. CARGAR DATOS Y MOSTRAR APP
        setTimeout(() => {
            loadFoliosFromFirebase().then(() => {
                console.log("Folios cargados:", folios.length);
                
                // Ocultar loading
                ocultarLoadingInicio();
                
                // 5. RESTAURAR VISIBILIDAD DE LA APP
                restaurarVisibilidadApp();
...
function mostrarLoadingInicio() {
    console.log("Mostrando loading de inicio...");
    
    // Limpiar loadings previos
    const existing = document.getElementById('loading-overlay-inicio');
    if (existing && existing.parentNode) {
        existing.parentNode.removeChild(existing);
    }
    
    const loadingOverlay = document.createElement('div');
    loadingOverlay.id = 'loading-overlay-inicio';
    loadingOverlay.className = 'loading-overlay inicio-sesion';
    ...
    // SOLO bloquear scroll del body (no del html)
    document.body.style.overflow = 'hidden';
...
function ocultarLoadingInicio() {
    const loadingOverlay = document.getElementById('loading-overlay-inicio');
    if (loadingOverlay) {
        loadingOverlay.classList.remove('active');
...
function initializeGastosOficina() {
    ...
    container.innerHTML = `
        <div class="section-card" style="background: var(--dark-card); border: 1px solid var(--dark-border); border-radius: 12px; padding: 25px; box-shadow: 0 4px 20px rgba(0,0,0,0.08); margin-bottom: 25px;">
            <div class="section-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; padding-bottom: 15px; border-bottom: 2px solid #e5e7eb;">
                <div>
                    <h3 style="margin: 0; color: #c0c1c2ff; font-size: 20px; display: flex; align-items: center; gap: 10px;">
                        <i class="fas fa-building" style="color: #c0c1c2ff;"></i>
                        Gastos de Oficina
                    </h3>
                    <p style="margin: 5px 0 0 0; color: #bec0c4ff; font-size: 14px;">Gastos administrativos y operativos</p>
                </div>
...
function loadGastosOficina() {
    ...
    gastos.forEach((gasto, index) => {
        ...
        html += `
            <tr id="gasto-${index}" style="transition: background 0.2s;">
                <td style="text-align: center; font-weight: 600; color: #6b7280;">${index + 1}</td>
                <td>
                    <div style="font-weight: 600; color: #1f2937;">${gasto.concepto || 'Sin concepto'}</div>
                </td>
                <td>
                    <div style="color: #4b5563;">${gasto.descripcion || 'Sin descripción'}</div>
                </td>
                <td>
                    <div style="color: #6b7280;">${gasto.fecha || 'No especificada'}</div>
                </td>
                <td>
    ...
    totalFooter.innerHTML = `
        <tr style="background: #f8fafc;">
            <td colspan="4" style="text-align: right; padding: 15px; font-weight: 600; color: #374151;">
                TOTAL GASTOS OFICINA:
            </td>
            <td style="text-align: right; padding: 15px; font-weight: 700; color: #cacacaff; font-size: 16px;">
        ...
        <div class="reporte-total-container" style="background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.08);">
            ...
            <div style="padding: 30px;">
                ...
                <div style="margin-bottom: 30px;">
                    <h3 style="color: #1f2937; margin: 0 0 20px 0; padding-bottom: 10px; border-bottom: 2px solid #e5e7eb; display: flex; align-items: center; gap: 10px;">
                        <i class="fas fa-users"></i>
                        Totales por Técnico
                    </h3>
    ...
    html += `
                ...
                <div style="background: #f0f9ff; border-radius: 10px; padding: 25px; margin-bottom: 30px; border: 1px solid #bae6fd;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px;">
                        <div>
                            <h4 style="color: #374151; margin: 0 0 15px 0; font-size: 16px;">Instalaciones</h4>
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // También cerrar menú al hacer clic en un enlace dentro del menú (mejora UX)
    document.querySelectorAll('#side-menu a, #side-menu button').forEach(item => {
        item.addEventListener('click', function() {
            ...
            }, 100); // Pequeño delay para permitir la navegación
        });
    });

    // Configurar logo/home también
    const homeLogo = document.querySelector('.app-title, .logo-home');
    if (homeLogo) {
    ...
    try {
        ...
        if (savedUser) {
            ...
            agregarEstilosBusquedaMejorada();
